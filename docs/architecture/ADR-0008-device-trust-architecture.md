...
| **3** | TPM seal + attestation + TPM-bağlı kimlik/enrollment + anti-rollback Tier-2 kaynağı imzalı config'e (ADR-0009) | RT-2, RT-3, RT-6 | swtpm ile | **Uygulandı** (yazılım+birim+contract; swtpm QEMU senaryoları ve G5 donanım kanıtı kaldı — ADR-0009) |
| **4** | Ağ yüzeyi: egress destination-set, firewall default, agent capability mediation | NEW-2, NEW-4, NEW-5 | Hayır | **Uygulandı** (NEW-2 #84; NEW-4 device-strip + watchdog paketleme; NEW-5 VARIANT-çapalı locked-by-default) |
