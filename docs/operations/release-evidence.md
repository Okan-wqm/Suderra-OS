...
| Field | Purpose |
|---|---|
| `schema_version` | Must be `suderra.release-evidence.v6` for promotion evidence; v2/v3/v4/v5 are archive-only. |
...
| `subject_graph` | Canonical `suderra.release-subject-graph.v1` binding for version, target, source run, artifact digest, and evidence roles. |
...
| `governance_role_bindings` | Role-to-GitHub identity binding manifest; role names alone are never identities. |
...
| `runtime_observations` | First-class typed runtime observations lifted from the preserved runtime suite. |
| `hardware` | Board serial logs and hardware acceptance results. |
| `hardware_subjects` | `suderra.hardware-subject.v1` records binding station acquisition, readback, device identity, and artifact digest to the release subject. |
...
| `signing_manifests` | `suderra.signing-manifest.v2` records binding HSM replay, artifact role, final digest, verifier command, and subject ID. |
...
reports, QEMU logs, OTA artifact records, subject graphs, signing manifests,
hardware subjects, governance role bindings, retention manifests, and approval
records. It preserves the original preflight input JSON/log/report files in the
target bundle and `--require-pass
...
The release workflow also downloads the latest successful signed
`governance-drift.yml` evidence from `main`; stale, unsigned, or failing drift
evidence blocks release even when the per-release snapshot passes.

In CI, `Release Preflight` treats audit evidence as an input source, not a
self-generated assertion. `collect-governance.py` may read a
`SUDERRA_GOVERNANCE_AUDIT_LOG` source export supplied by ingress, or fetch the
GitHub audit log through the governed API and preserve the raw export plus a
replayable `audit-log.json`. A missing, failed, stale, or self-referential audit
record fails closed.
