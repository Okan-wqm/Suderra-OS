...
- Release input preflight passes before any build job starts. It validates
  ...
  approvals, reproducibility logs, and security reports for the exact tag.
...
- Full-matrix lab input exists under `release-lab-input/<version>/<target>/`
  for hardware targets and validates as `suderra.lab-evidence.v2`.
- QEMU targets provide `release-lab-input/<version>/<target>/qemu.json` and
  validate as `suderra.qemu-acceptance.v2`.
...
Do not tag until governance is active and required release input is committed or
otherwise available at the expected paths. The release workflow runs only from
`refs/tags/v*`; branch `workflow_dispatch` is intentionally not available for
release publication. The normal CI build workflow does not run on release tags,
so the release workflow owns the exact-tag build and evidence path.
