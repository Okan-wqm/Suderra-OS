...
- GitHub rulesets, `main` branch protection, tag protection, and the
  `release-publish` environment are active and exported as governance evidence.
...
- Full-matrix lab input exists under `release-lab-input/<version>/<target>/`
  for hardware targets and validates with `validate-lab-input.py`.
- The release workflow stages the exact release bytes, writes
  ...
  staged bytes.
- Final `release-evidence/<version>/<target>/evidence.json` validates with
...
Do not tag until governance is active and hardware lab input is committed.
Final release evidence is generated by the tag workflow from the staged,
signed, and attested release bytes. A tag may not be published if the workflow
cannot bind the final artifact hashes, signatures, attestations, lab input, and
governance snapshots into `release-evidence/<version>/`.
