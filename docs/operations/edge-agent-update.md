...
kapalıdır. QEMU/lab için agent'ı OS image içine gömmek gerekirse:
...
```bash
make -C buildroot BR2_EXTERNAL=/var/suderra-os/Suderra-OS \
  O=/tmp/suderra-agent-sourcecheck suderra_qemu_x86_64_defconfig
make -C buildroot BR2_EXTERNAL=/var/suderra-os/Suderra-OS \
  O=/tmp/suderra-agent-sourcecheck suderra-edge-agent-source
sha256sum buildroot/dl/suderra-edge-agent/suderra-edge-agent-<sha>-cargo2.tar.gz
```
