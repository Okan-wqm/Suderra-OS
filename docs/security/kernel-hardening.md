...
- [ ] `board/suderra/common/rootfs-overlay/etc/security/limits.d/90-suderra.conf` (ulimit — düşük öncelik: tek iş yükünün limitleri systemd unit'te)
- [x] Statik hardening baseline → `tests/security/lynis-baseline.sh` (token denetimi; CI'da koşuyor)
- [ ] Gerçek Lynis runtime taraması imaj üstünde (QEMU'da boot edip lynis audit — henüz yok)
