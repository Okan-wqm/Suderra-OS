...
write(qemu_root / "serial.log", "serial boot evidence\n")
write(qemu_root / "qmp.json", "qmp events\n")
qemu_checks = {
    name: {"status": "passed"}
    for name in (
        "boot",
...
for target, boards in boards_by_target.items():
    ...
    for board in boards:
        ...
        write(lab_root / log, f"{board} serial transcript\n")
        checks = {}
        ...
        for check in names:
            ...
            write(lab_root / evidence, f"{board} {check} evidence\n")
            checks[check] = {"status": "passed", "evidence": evidence}
        devices.append(
            {
                "board": board,
    ...
    if target == "pi-cm4-revpi-usb-installer":
        for name in negative_tests:
            ...
            write(lab_root / evidence, f"{name} closed-fail evidence\n")
...
for target in ("qemu-x86_64", "rpi4", "pi-cm4-revpi-usb-installer", "revpi4"):
    write_json(
        ...
        {"status": "approved", "approver": "contract", "approved_at": "2026-05-13T00:00:00Z"},
    )
